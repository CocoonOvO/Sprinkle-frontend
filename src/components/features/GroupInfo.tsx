      ...
      <div className="p-4">
        <div className="flex flex-col items-center gap-4 py-8">
          {loadingMembers ? (
            ...
            <>
              <Avatar size="xl" />
              <p className="text-stone-500 text-sm">暂无群信息</p>
            </>
          )}
        </div>
      </div>
    ...
    <div className="p-4 space-y-6">
          ...
          {editingName ? (
            <div className="flex items-center gap-2">
              <input
                ...
                className={cn(
                  'px-2 py-1 text-center font-semibold rounded-lg border border-strawberry-200',
                  'focus:outline-none focus:ring-2 focus:ring-strawberry-500/30'
                )}
                autoFocus
              />
              <button onClick={handleSave} disabled={saving} className="p-1 text-strawberry-600">
                <Check className="w-4 h-4" />
              </button>
              <button onClick={() => setEditingName(false)} className="p-1 text-stone-400">
                <X className="w-4 h-4" />
              </button>
            </div>
          ) : (
            <div className="flex items-center justify-center gap-2">
              <h3 className="font-semibold text-stone-800">{currentConversation.name || '未命名群组'}</h3>
              <button
                onClick={() => {
                  setNameValue(currentConversation.name || '');
                  setEditingName(true);
                }}
                className="p-1 text-stone-400 hover:text-strawberry-600 transition-colors"
              >
                <Edit3 className="w-3 h-3" />
              </button>
            </div>
          )}
          <p className="text-sm text-stone-500">{currentConversation.member_count} 位成员</p>
        </div>
      </div>

      <div className="space-y-3">
        <label className="text-sm font-medium text-stone-700">群介绍</label>
        {editingDesc ? (
          ...
          <div
            ...
            className={cn(
              'p-3 bg-stone-50 rounded-xl text-sm cursor-pointer',
              'hover:bg-stone-100 transition-colors',
              !currentConversation.metadata?.description && 'text-stone-400'
            )}
          >
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <label className="text-sm font-medium text-stone-700">群聊提示词</label>
          {!editingPrompt ? (
            <button
              onClick={() => {
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={handleSave}
                disabled={saving}
                className="p-1 text-strawberry-600 hover:text-strawberry-700"
              >
                <Check className="w-4 h-4" />
              </button>
              <button
        ...
        <p className="text-xs text-stone-400">设置群聊的默认提示词</p>
        {editingPrompt ? (
          <div className="space-y-2">
            <textarea
              value={promptValue}
            ...
            />
            <div className="flex items-center justify-between">
              <span className="text-xs text-stone-400">{promptValue.length} / 2000 字</span>
              <div className="flex items-center gap-2">
          ...
          <div
            className={cn(
              'p-3 bg-stone-50 rounded-xl text-sm',
              !currentConversation?.metadata?.prompt && 'text-stone-400'
            )}
          >
      ...
      <div className="space-y-3">
        <label className="text-sm font-medium text-stone-700">群头像</label>
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm text-stone-500">点击头像更换</p>
            <p className="text-xs text-stone-400">支持 JPG、PNG，建议 200x200</p>
          </div>
        </div>
      </div>
    </div>
